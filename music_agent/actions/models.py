...
class MusicAction(BaseModel):
    ...
    name: MusicActionName
    params: dict[str, Any] = Field(default_factory=dict)
    confidence: float = Field(ge=0.0, le=1.0)

    @model_validator(mode="after")
    def validate_params(self) -> "MusicAction":
        ...
        if self.name == "adjust_volume":
            delta = float(self.params.get("delta"))
            if delta < -1.0 or delta > 1.0 or delta == 0:
                raise ValueError("Volume delta must be between -1 and 1, excluding 0.")
